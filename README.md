...
67通常テーブル、migration `0001`〜`0031`、147経路の契約があり、主要なFiles REST/WebDAV mutation、trash/restore/purge、fenced R2 GC、content ticket/blob配信、private単一・分割アップロードまでローカル接続しています。
...
[共通の更新受付](docs/MUTATION_ADMISSION.md)は、namespace・DAVロック・app password発行/失効/pepper更新を同時32件・待機256件で制御します。資格情報の変更も確定記録・枠解放と一括保存します。sessionやupload準備など残る経路への接続は続けて開発しています。
