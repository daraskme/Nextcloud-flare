...
export function ShareDialog({ item, onClose }: ShareDialogProps): React.JSX.Element {
  ...
  return (
    <div className="fixed inset-0 z-[60] grid place-items-center bg-[var(--overlay)] p-4 backdrop-blur-sm">
      <section className="w-full max-w-lg overflow-hidden rounded-3xl border border-[var(--border)] bg-[var(--surface)] shadow-2xl shadow-black/30">
        <header className="flex items-center justify-between border-b border-white/[0.08] px-6 py-5">
          <div>
            ...
            <h2 className="mt-1 max-w-sm truncate text-lg font-semibold text-[var(--fg)]">
              {item.name}
            </h2>
          </div>
          <button
        ...
        {created !== null ? (
          <div className="space-y-5 p-6">
            ...
            <div>
              <h3 className="text-lg font-medium text-[var(--fg)]">{t("share.ready")}</h3>
              <p className="mt-1 text-sm text-slate-400">
                {created.kind === "link"
                  ? t("share.secret")
                  : `${t("share.sharedRecipient")} ${created.granteeEmail ?? ""}`}
              </p>
            </div>
          ...
          <div className="space-y-5 p-6">
            <div className="grid grid-cols-2 gap-2 rounded-2xl bg-black/20 p-1.5">
              <button
                className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm transition ${kind === "link" ? "bg-white/10 text-[var(--fg)] shadow" : "text-slate-500 hover:text-slate-300"}`}
                onClick={() => setKind("link")}
              >
                <Link2 className="h-4 w-4" /> {t("share.linkType")}
              </button>
              <button
                className={`flex items-center justify-center gap-2 rounded-xl px-3 py-2.5 text-sm transition ${kind === "user" ? "bg-white/10 text-[var(--fg)] shadow" : "text-slate-500 hover:text-slate-300"}`}
                onClick={() => {
                  setKind("user");
                  if (mode === "upload") setMode("view");
                }}
              >
                <UserRound className="h-4 w-4" /> {t("share.person")}
              </button>
            </div>
            ...
            {kind === "user" && (
              <label className="block text-xs font-medium text-slate-400">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3 text-sm text-[var(--fg)] outline-none transition focus:border-sky-400/50"
                />
              </label>
            )}
            ...
            <div className="grid gap-4 sm:grid-cols-2">
              <label className="text-xs font-medium text-slate-400">
                ...
                <input
                  ...
                  className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-3 py-2.5 text-sm text-[var(--fg)] outline-none focus:border-sky-400/50"
                />
              </label>
              {kind === "link" && (
                <label className="text-xs font-medium text-slate-400">
                  ...
                  <span className="relative mt-2 block">
                    ...
                    <input
                      ...
                      className="w-full rounded-xl border border-white/10 bg-black/20 py-2.5 pl-10 pr-3 text-sm text-[var(--fg)] outline-none focus:border-sky-400/50"
                    />
                  </span>
                </label>
              )}
