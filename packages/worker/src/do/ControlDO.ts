...
export class ControlDO extends DurableObject<Env> {
  constructor(ctx: DurableObjectState, env: Env) {
    ...
    ctx.storage.sql.exec(`CREATE TABLE IF NOT EXISTS recovery_audit_v6(
      singleton INTEGER PRIMARY KEY CHECK(singleton=1),epoch INTEGER NOT NULL,
      token TEXT NOT NULL,stage TEXT NOT NULL CHECK(stage IN ('users','blobs','r2','outbox','shares','credentials','credential_sources','fts','complete')),
  ...
  #auditRow(expectedEpoch: number): AuditRow {
    const row = this.ctx.storage.sql
      .exec<AuditRow>(
        "SELECT epoch,token,stage,after_id,pages FROM recovery_audit_v6 WHERE singleton=1",
      )
      .toArray()[0];
  ...
  async beginRecoveryAudit(expectedEpoch: number): Promise<RecoveryAuditStatus> {
    ...
    this.ctx.storage.sql.exec(
      `INSERT INTO recovery_audit_v6(singleton,epoch,token,stage,after_id,pages)
