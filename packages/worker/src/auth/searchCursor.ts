...
function valid(value: SearchCursorClaims, now: number): boolean {
  return (
...
/** Search-only cursor binds normalized query, scope, authority and tree generation. */
export class SearchCursorTokens {
  constructor(
    readonly ring: ContentKeyRing,
    readonly now: () => number = Date.now,
  ) {}

  async issue(claims: Omit<SearchCursorClaims, "iat" | "exp">): Promise<string> {
    const now = Math.floor(this.now() / 1000);
    const payload: SearchCursorClaims = { ...claims, iat: now, exp: now + 600 };
    if (!valid(payload, now)) throw new Error("invalid_search_cursor");
    const kid = this.ring.activeKid;
    const key = this.ring.keys.get(kid);
    if (!key) throw new Error("invalid_search_cursor");
    const header = base64url.encode(
      JSON.stringify({ alg: "HS256", kid, typ: "ncf-search-cursor" }),
    );
    ...
    if (token.length > 4096) throw new Error("invalid_search_cursor");
    return token;
  }

  async verify(token: string): Promise<SearchCursorClaims> {
    try {
      ...
      if (
        ...
        headerText !==
          base64url.encode(
            JSON.stringify({ alg: "HS256", kid: header.kid, typ: "ncf-search-cursor" }),
          )
      )
        throw new Error("invalid_search_cursor");
