...
export interface NodeCursorClaims {
...
export class NodeCursorTokens {
  constructor(
    readonly ring: ContentKeyRing,
    readonly now: () => number = Date.now,
  ) {}

  async issue(claims: Omit<NodeCursorClaims, "iat" | "exp">): Promise<string> {
    const now = Math.floor(this.now() / 1000);
    const payload: NodeCursorClaims = { ...claims, iat: now, exp: now + 600 };
    if (!valid(payload, now)) throw new Error("invalid_node_cursor");
    const kid = this.ring.activeKid;
    const key = this.ring.keys.get(kid);
    if (!key) throw new Error("invalid_node_cursor");
    const header = base64url.encode(JSON.stringify({ alg: "HS256", kid, typ: "ncf-node-cursor" }));
    ...
    if (token.length > 4096) throw new Error("invalid_node_cursor");
    return token;
  }

  async verify(token: string): Promise<NodeCursorClaims> {
    try {
      ...
      if (
        ...
        headerText !==
          base64url.encode(
            JSON.stringify({ alg: "HS256", kid: header.kid, typ: "ncf-node-cursor" }),
          )
      )
        throw new Error("invalid_node_cursor");
