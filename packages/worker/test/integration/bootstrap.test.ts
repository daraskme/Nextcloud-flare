...
it("selects exactly one bootstrap identity under competing owner logins, with one complete personal space", async () => {
  ...
  const results = await Promise.allSettled([
    bootstrapOwner(db, one, 1, policy),
    bootstrapOwner(db, two, 1, policy),
  ]);
  ...
  const result = await bootstrapOwner(db, claims, 1, policy);
...
it("rejects outsiders, maintenance, old epoch and expired tokens without partially consuming bootstrap", async () => {
  ...
  await expect(bootstrapOwner(db, outsider, 1, policy)).rejects.toThrow("bootstrap_not_allowed");
  await expect(bootstrapOwner(db, claims, 2, policy)).rejects.toThrow("bootstrap_unavailable");
  await db.prepare("UPDATE control SET maintenance=1").run();
  await expect(bootstrapOwner(db, claims, 1, policy)).rejects.toThrow();
  await db.prepare("UPDATE control SET maintenance=0").run();
  await expect(bootstrapOwner(db, { ...claims, exp: 1 }, 1, policy)).rejects.toThrow();
...
it("connects verified JWT, bootstrap and durable sessions without implicit signup or email identity merge", async () => {
  ...
  const sessions = await Promise.all([
    loginAccessUser(db, access.verifier, request, 1, policy),
    loginAccessUser(db, access.verifier, request, 1, policy),
  ]);
  expect(sessions[0]?.credential_id).toBe(sessions[1]?.credential_id);
  await expect(
    loginAccessUser(db, access.verifier, await access.sign({ sub: "other-identity" }), 1, policy),
  ).rejects.toThrow();
  const session = sessions[0];
  if (!session) throw new Error("missing_session");
  await revokeAccessSession(db, session.credential_id, 1);
  await expect(loginAccessUser(db, access.verifier, request, 1, policy)).rejects.toThrow();
});
...
it("reconciles a committed bootstrap after its batch response is lost", async () => {
  ...
  const claims = await access.verifier.verify(await access.sign(), "user");
  await expect(bootstrapOwner(lossy, claims, 1, policy)).resolves.toMatchObject({
