...
export async function seedFoundation(now = Date.now()): Promise<void> {
  ...
  await env.DB.batch([
    ...
    env.DB.prepare("DELETE FROM node_stars"),
    env.DB.prepare("DELETE FROM node_tags"),
    env.DB.prepare("DELETE FROM media_jobs"),
    env.DB.prepare("DELETE FROM node_media"),
    env.DB.prepare("DELETE FROM audio_jobs"),
    env.DB.prepare("DELETE FROM node_audio"),
    env.DB.prepare("DELETE FROM library_jobs"),
    env.DB.prepare("DELETE FROM library_items"),
    env.DB.prepare("DELETE FROM library_roots"),
    env.DB.prepare("DELETE FROM node_props"),
    env.DB.prepare("DELETE FROM content_sessions"),
    env.DB.prepare("DELETE FROM content_tickets"),
    env.DB.prepare("DELETE FROM zip_manifests"),
    ...
    env.DB.prepare("INSERT INTO search_fts(search_fts) VALUES('delete-all')"),
