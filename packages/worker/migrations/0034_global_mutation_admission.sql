-- Ownerless internal updates share the existing FIFO and capacity. Apply only while drained.
-- Rebuild CHECK constraints without changing historical receipts or AUTOINCREMENT high-water.

INSERT INTO _assert(v) SELECT 1 WHERE NOT EXISTS(SELECT 1 FROM control WHERE singleton=1 AND maintenance=1) OR EXISTS(SELECT 1 FROM mutation_admissions WHERE state<>'closed') OR EXISTS(SELECT 1 FROM permits WHERE state='open') OR EXISTS(SELECT 1 FROM operations WHERE state='claimed');
...
DROP TRIGGER mutation_commit_insert;
...
CREATE TABLE mutation_admissions_next(
 ...
 space_id TEXT REFERENCES spaces(id) CHECK(space_id IS NOT NULL OR (system=0 AND length(permit_id)=46 AND substr(permit_id,1,10)='bootstrap:') OR (system=1 AND substr(permit_id,1,7)='global:')),
 ...
 granted_at INTEGER,
 expires_at INTEGER,
 committed_at INTEGER CHECK(committed_at IS NULL OR (committed_at BETWEEN 0 AND 9007199254740991 AND state='closed' AND granted_at IS NOT NULL AND committed_at>=granted_at AND committed_at<expires_at)), system INTEGER NOT NULL DEFAULT 0
 CHECK(system IN (0,1) AND (system=0 OR (space_id IS NOT NULL AND substr(permit_id,1,7)='system:') OR (space_id IS NULL AND substr(permit_id,1,7)='global:'))), maintenance INTEGER NOT NULL DEFAULT 0
...
INSERT INTO mutation_admissions_next(seq,id,permit_id,space_id,epoch,requested_at,wait_until,state,granted_at,expires_at,committed_at,system,maintenance) SELECT seq,id,permit_id,space_id,epoch,requested_at,wait_until,state,granted_at,expires_at,committed_at,system,maintenance FROM mutation_admissions;

INSERT INTO sqlite_sequence(name,seq) SELECT 'mutation_admissions_next',seq FROM sqlite_sequence WHERE name='mutation_admissions' AND NOT EXISTS(SELECT 1 FROM sqlite_sequence WHERE name='mutation_admissions_next');

UPDATE sqlite_sequence SET seq=MAX(seq,COALESCE((SELECT seq FROM sqlite_sequence WHERE name='mutation_admissions'),0)) WHERE name='mutation_admissions_next';
...
WHEN (NEW.state='open' AND substr(NEW.permit_id,1,7) IN ('system:','global:')) OR (
