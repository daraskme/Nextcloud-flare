...
try {
  ...
  const arrayJson = (values) => JSON.stringify(values, null, 2).replace(/"\n\]$/, '",\n]');
  await writeFile(
    ...
    `// Generated by scripts/generate-schema-contracts.mjs; do not hand-order.\n` +
      `export const purgeOrder = ${arrayJson(order)} as const;\n` +
      `// FTS virtual/shadow tables are rebuilt from search_index after export restoration.\n` +
      `export const exportTables = ${arrayJson(tables.map((table) => table.name))} as const;\n`,
  );
  console.log(
