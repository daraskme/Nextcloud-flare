...
最新の全検証記録は Node 242件 + workerd 561件 = 803件で、lint、typecheck、contracts、config、schema整合性テスト、Web build、Wrangler dry-runを含む。migration `0020`をローカルD1/SQLiteへ適用済み。件数は追加実装で変わるため、次回は再実行結果で更新する。
...
1. unknown multipart IDと未知R2 objectの隔離・inventory・修復を実装。既知IDの回収とprivate HTTPは接続済み。
