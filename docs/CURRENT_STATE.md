...
直近の到達点と進行中の作業は[PROGRESS](PROGRESS.md)へ記録する。直前commit `026f534`はpush済みで、[CI](https://github.com/daraskme/Nextcloud-flare/actions/runs/35993638387)のUbuntu・Windows・browser全jobが成功した。
...
| KDF実行制限 | Worker/ControlDO各1件・待機256件・5秒、D1の600回/65秒予算と未精算20枠、epoch cooldown、発行/認証/鍵更新と503応答 | 新規Node7件・workerd20件、既存認証34件、実ControlDO RPC/eviction/全喪失。詳細は[KDF_ADMISSION](KDF_ADMISSION.md) | 未精算repair、共有password/IP制限、実CPU・処理量・切断 |
...
| 復旧基盤 | epoch履歴、quiesce、paged recovery audit、FTS rebuild、限定cleanup、受付/GCの段階再開、永続repair hold | DO eviction/全喪失、実LockDO mutation、HTTP bootstrap、応答喪失・停止競合、最終batch fence | 完全restore drill、実環境、account mutation・KDF未精算repair |
...
直前commit `026f534`はNode389 + workerd857 + browser19 = **1,265件**、Ubuntu・Windows・browserのCI全成功。今回のKDF制限を含む全checkはNode396 + workerd877 = **1,273件**成功。新規workerd20件の最終再検証も成功。browser19件も成功し、合計**1,292件**。確定結果は[IMPLEMENTATION_STATUS](IMPLEMENTATION_STATUS.md)へ記録する。
...
- account単位のmutation同時数・待ちqueue、KDF未精算のrepairと共有password/IP制限、backup専用barrier。KDFの全体rate/枠とisolate内制限は接続済み。
...
4. account mutation / KDF未精算repair、backup barrier、実環境のrestore/再開drill。内部RPCの段階再開は実装済み。
