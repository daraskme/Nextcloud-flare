...
停止・epoch更新・repairは過去の監査を無効化する。`resumeAdmission`の再送は既にactiveなら現在状態を返し、GC設定を勝手に変更しない。GC操作は受付再開前には実行できない。監査pageはactive/opening中に進められない。
...
DO SQLiteの`control_admission`にepoch/revision/token、遷移中phase、直前token、GC目標値、監査tokenを保存してからD1へアクセスする。D1/R2を待っている間の`status()`は受付停止を返す。open状態でもD1の同じepoch/revision/token/flagsを確認し、その応答後にDOの状態を再確認する。
