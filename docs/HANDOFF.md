...
Phase 0 のローカル基盤、Phase 1 の大半と Phase 2 / WebDAV の一部。54通常テーブル、migration `0001`〜`0013`、147 route の契約がある。
...
- UploadDO、Files UI、uploadの本実装、trash purge/GC、全 operation の認可 tuple、検索/共有/contentの残り、DAV実client gate、Gallery/Bookshelf/Audio、運用・release は未完了。trash一覧と同期restoreは接続済み。実 ControlDO admission とリモート Access/署名鍵設定、全 route 会計も未接続。
