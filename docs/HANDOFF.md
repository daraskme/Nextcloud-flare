...
今回の追加: 復旧監査に share/credential ページを追加し、予約量、root 所有者、version、credential 参照先種別と有効 scope root を検査する。既存 DO の旧監査テーブルの CHECK 制約を避けるため進捗を `recovery_audit_v3` に保存する。NixOS の Node 24.20.0 / pnpm 12.3.4 で `pnpm check` **439 tests**（Node 195、workerd 244）と lint/typecheck/contracts/config/build に成功。監査完了は再開の証明ではなく、ControlDO admission は閉じたまま。前回の `c02a9b2` の [CI](https://github.com/daraskme/Nextcloud-flare/actions/runs/35687077255) も成功。
...
2. **ControlDO admission / resume**: durable な監査進捗へ FTS、credential/share/outbox の全意味検証と逆向き参照検査、R2 inventory、GC/Upload/Queue drain、最終 D1 fence を追加する。正本は DO、D1 は mirror。空 DB 専用の解除処理を完成形にしない。
