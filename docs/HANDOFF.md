...
今回の追加: 停止中の `search_index` から FTS5 を再構築する RPC と、元テーブルを含む `integrity-check` を最終監査段階に追加。再構築後は監査を最初からやり直す。既存 DO の旧監査テーブルの CHECK 制約を避けるため進捗を `recovery_audit_v4` に保存する。NixOS の Node 24.20.0 / pnpm 12.3.4 で `pnpm check` **440 tests**（Node 195、workerd 245）と lint/typecheck/contracts/config/build に成功。監査完了は再開の証明ではなく、ControlDO admission は閉じたまま。前回の `6670df1` の [CI](https://github.com/daraskme/Nextcloud-flare/actions/runs/35687711951) も成功。
...
2. **ControlDO admission / resume**: durable な監査進捗へ credential/share/outbox の全意味検証と逆向き参照検査、R2 inventory、GC/Upload/Queue drain、最終 D1 fence を追加する。正本は DO、D1 は mirror。空 DB 専用の解除処理を完成形にしない。
