...
| 1 復旧監査ページ | D1 quiesce、bootstrap/admin/root、owner ledger/ref、R2 HEAD size/etag、outbox provenance/lease、share 予約量・root・version、credential 参照先・有効 scope root を各最大20件ずつ検証。ControlDO SQLite の epoch/token/cursor 永続化、eviction・失敗ページ再試行・旧 epoch 拒否を実証 | 読み取り専用監査。FTS、credential/share/outbox の全意味検証と逆向き参照検査、R2 inventory と最終再開 gate は未完了 |
...
- **I**: Windows と NixOS のローカル workerd binding テスト。初回 CI の Windows 改行失敗を `.gitattributes` で修正し、[2fd68ac の CI](https://github.com/daraskme/Nextcloud-flare/actions/runs/35629022538) は Windows/Ubuntu 両方で成功（media 込み350 tests 時点）。今回の439 tests と前回の437/436/433/429/426/422/415 tests は以下のローカル実行記録。最新 HEAD の CI は GitHub Actions で照合する。
