...
| 1 復旧監査ページ | D1 quiesce、bootstrap/admin/root、owner ledger/ref、R2 HEAD size/etag を user/blob 各最大20件ずつ検証。破損/欠損とページ境界を D1/R2 で実証 | 読み取り専用 helper。durable 進捗・FTS/credential/share/outbox/R2 inventory と再開 gate は未完了 |
...
- **I**: Windows と NixOS のローカル workerd binding テスト。初回 CI の Windows 改行失敗を `.gitattributes` で修正し、[2fd68ac の CI](https://github.com/daraskme/Nextcloud-flare/actions/runs/35629022538) は Windows/Ubuntu 両方で成功（media 込み350 tests 時点）。今回の433 tests と前回の429/426/422/415 tests は以下のローカル実行記録。最新 HEAD の CI は GitHub Actions で照合する。
