...
| 6 content blob read 基盤 | 署名 ticket の D1 現行検査→発行元 ticket 束縛 content session→署名 Cookie→node/blob/R2 manifest と current credential を同一 D1 batch で検証。鍵 rotation と個別 cancel を反映 | private/匿名 share の実 D1/R2、失効・share version・hash 競合、復旧監査で検証。BudgetDO、HTTP/CORS 公開 route は未接続 |
