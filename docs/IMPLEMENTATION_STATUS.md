...
| 7 DAV existing-resource lock | bounded `lockinfo` XML、Depth、Timeout、既存 node の exclusive write LOCK、空 body + `If` token の refresh、UNLOCK を HTTP/LockDO/D1 に接続。token は hash のみ保存し、refresh/UNLOCK は同 user の app password と current write 認可を再検証。PROPFIND `supportedlock` は exclusive write を返す。LOCK公開前に同spaceのactive permit不存在を原子的に検査し、期限切れpermitとclaimを収束 | 200/204、lock/permit競合423、期限切れpermit回収、条件不一致412、token再利用409、timeout最大1時間を実D1で検証。lock-null、active `lockdiscovery`、残るmutation/COPY/MOVEが未完了なので Class 2 は未広告 |
