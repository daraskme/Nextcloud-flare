...
| 7 DAV conditions | RFC 4918 `If` の tagged/untagged、condition AND、全 list production OR、`Not`、state token、strong/weak ETag と独立 token submission、単一 `Lock-Token` を bounded parser/evaluator に実装 | 8 KiB、resource tag/list 各16、全64・各list16 condition、token/ETag/URI長を unit fixture で検証。same-origin current D1 path/ancestor lock/ETag stateを評価し、提出tokenをMKCOL/PROPPATCHのLockDO/D1 assertionへ接続。不一致412、malformed 400、条件が常真でも必要token未提出は423。`Lock-Token` はUNLOCK以外で400 |
| 7 DAV existing-resource lock | bounded `lockinfo` XML、Depth、Timeout、既存 node の exclusive write LOCK、空 body + `If` token の refresh、UNLOCK を HTTP/LockDO/D1 に接続。token は hash のみ保存し、refresh/UNLOCK は同 user の app password と current write 認可を再検証。PROPFINDはexclusive write `supportedlock` と、直接・祖先infinity lockのscope/type/depth/owner/残timeout/取得時lockrootを返しtokenは非表示。LOCK公開前に同spaceのactive permit不存在を原子的に検査し、期限切れpermitとclaimを収束 | 200/204、lock/permit競合423、期限切れpermit回収、条件不一致412、token再利用409、timeout最大1時間、直接/継承`lockdiscovery`を実D1で検証。lock-nullと残るmutation/COPY/MOVEが未完了なので Class 2 は未広告 |
