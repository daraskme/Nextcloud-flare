...
`api/nodeMutations.ts` は app の `POST /api/v1/nodes` を folder 専用の bounded JSON、CSRF、Idempotency-Key で `createFolder` に接続する。terminal commit は201、failed/conflict は409、未確定は Operation-Id/Retry-After 付き503。同 credential の `GET /api/v1/operations/:id` は既存の current operand/result 照合を使う。実 LockDO/D1 の HTTP テストは test-only admission を使用し、ControlDO の実再開を意味しない。
