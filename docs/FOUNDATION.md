...
`api/contentTickets.ts` は認証済み private request の CSRF、同一 origin、bounded JSON を確認し、発行と取消を内部サービスへ渡す。Worker entry の Access verifier/session 接続は未実装で、この handler はまだ公開されていない。
