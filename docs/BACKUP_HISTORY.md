...
これはオフライン検証の互換性と抽出漏れの防止であり、元BLOBSの実在性、Time Travel/live restore、新epochと復旧監査、定時起動/期限切れ世代の自動回収、remoteの運用検証は別工程である。日次コマンドと[保持判定](BACKUP_RETENTION.md)は接続済みである。
